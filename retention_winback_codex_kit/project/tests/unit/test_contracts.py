...
def test_cutoff_purchase_presence_and_inactive_buyer():
    ...
    c.execute(
        "CREATE TABLE int_behavior_events(user_id VARCHAR,event_time TIMESTAMPTZ,event_type VARCHAR,price DOUBLE,valid_price BOOLEAN,user_session VARCHAR,product_id VARCHAR,category_id VARCHAR)"
    )
    rows = [
        ("a", "2019-11-01", "purchase", None, False, "s", "p", "9000000000000000001"),
        ("a", "2019-12-01", "purchase", 10, True, "s", "p", "future"),
        ("b", "2019-11-05", "purchase", 5, True, None, "p", None),
        ("b", "2019-12-29", "purchase", 5, True, "s", "p", None),
    ]
    ...
    assert (
        c.execute(
            "SELECT count(*) FROM int_behavior_events WHERE user_id='a' AND event_time>='2019-12-01' AND event_time<'2019-12-29'"
        ).fetchone()[0]
        == 1
    )
    assert (
        c.execute(
            "SELECT count(*) FROM int_behavior_events WHERE user_id='b' AND event_time>='2019-12-01' AND event_time<'2019-12-29'"
        ).fetchone()[0]
        == 0
    )
    c.close()
...
def test_allowlist_partition_and_ties():
    for col in ["user_id", "non_repurchase_28d", "next_purchase_date", "visit"]:
        with pytest.raises(ValueError):
            validate_features([col])
    ids = np.array([str(i) for i in range(100)])
    mask = partition(ids)
    assert 0 < mask.sum() < 100
    assert set(ids[mask]).isdisjoint(ids[~mask])
    assert top_indices([0.5, 0.5], ["b", "a"], 0.5).tolist() == [1]
    assert len(top_indices([], [], 0.1)) == 0


def test_economics_and_assignment():
    assert economics()["value"] is None
    assert economics(0.1, 0.01, 100, 0.3, 5, 1, 0, count=100)[
        "per_contact"
    ] == pytest.approx(-0.25)
    with pytest.raises(ValueError):
        economics(0.99, 0.1, 100, 0.3, 5, 1, 0)
    assert not economics(0.1, 0.01, 1, 0.1, 5, 1, 0)["break_even_feasible"]
    assert power(0.1, 0.01)["n_control"] > 100
    a = assignments(["a", "b"])
    assert assignments(["c", "a"], a)["a"] == a["a"]
...
def test_segment_precedence_and_audience():
    df = pd.DataFrame(
        {
            "user_id": ["a", "b"],
            "purchase_recency": [1, 30],
            "valid_price_share": [1, 1],
            "value_60": [100, 10],
            "activity_change": [-5, 0],
            "views_28": [5, 0],
            "carts_28": [0, 0],
            "repeat_buyer": [True, False],
            "event_recency": [1, 30],
        }
    )
    result = segment(df, 50)
    assert result.primary_segment.tolist() == [
    ...
    for col in [
        ...
        result[col] = "test_fixture"
    result["historical_value_proxy"] = result.value_60
    result["risk_probability"] = 0.8
    result["risk_value_priority"] = 1
    result["contactability_status"] = "unknown"
    result["consent_status"] = "unknown"
