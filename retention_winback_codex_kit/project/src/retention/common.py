...
CUTS = {
    "train": "2019-12-01",
    "validation": "2019-12-30",
    "test": "2020-01-28",
    "latest_unlabeled": "2020-03-01",
}
...
def run_dir(mode: str) -> Path:
    if mode not in ("dev", "full"):
        raise ValueError("mode must be dev or full; fixtures never enter results")
    p = ROOT / "artifacts" / f"{mode}-v1"
    p.mkdir(parents=True, exist_ok=True)
    return p


def connect(mode: str, readonly: bool = False) -> duckdb.DuckDBPyConnection:
    p = ROOT / "warehouse"
    p.mkdir(exist_ok=True)
    con = duckdb.connect(str(p / f"{mode}.duckdb"), read_only=readonly)
    con.execute("SET TimeZone='UTC'")
    if not readonly:
        spill = ROOT / ".cache" / f"spill-{mode}"
        spill.mkdir(parents=True, exist_ok=True)
        con.execute(
            "SET memory_limit='4GB'; SET threads=4; SET enable_progress_bar=false; SET TimeZone='UTC'"
        )
