...
def trace(mode="dev"):
    p = run_dir(mode)
    scores = pd.read_parquet(p / "scores_test.parquet")
    diagnostic = pd.read_parquet(p / "test_diagnostics.parquet")
    records = diagnostic.merge(
        scores[
            ["user_id", "primary_segment", "purchase_recency", "purchasing_days_60"]
        ],
        on="user_id",
        validate="one_to_one",
    )
    ...
    for name, mask in [
        (
            "false_positive",
            (records.risk_probability >= 0.5) & records.non_repurchase_28d.eq(0),
        ),
        (
            "false_negative",
            (records.risk_probability < 0.5) & records.non_repurchase_28d.eq(1),
        ),
    ]:
        if mask.any():
            choices[name] = records[mask].sort_values("user_id").iloc[0]
    for i, row in records.sort_values("user_id").iloc[1:13].iterrows():
        choices[f"explorer_{row.user_id}"] = row
    ...
    for kind, row in choices.items():
        uid = str(row.user_id)
        t = pd.Timestamp(CUTS["test"], tz="UTC")
        events = con.execute(
            "SELECT source_file,source_row,event_time,event_type,product_id,category_id,price,valid_price,user_session,identical_row_count FROM int_behavior_events WHERE user_id=? AND event_time>=? AND event_time<? ORDER BY event_time,source_file,source_row",
            [uid, t - pd.Timedelta(days=60), t],
        ).df()
        purchases = events[events.event_type.eq("purchase")]
        manual = {
            "purchase_recency": (t - purchases.event_time.max()).total_seconds()
            / 86400,
            "purchasing_days_60": int(purchases.event_time.dt.date.nunique()),
            "value_60": float(purchases.loc[purchases.valid_price, "price"].sum()),
        }
        for key, value in manual.items():
            np.testing.assert_allclose(value, row[key], rtol=1e-8)
        events.to_parquet(directory / f"{uid}.parquet", index=False)
        future = con.execute(
            "SELECT source_file,source_row,event_time,event_type,price FROM int_behavior_events WHERE user_id=? AND event_time>=? AND event_time<? ORDER BY event_time,source_file,source_row",
            [uid, t, t + pd.Timedelta(days=28)],
        ).df()
        ...
        index.append(
            {
                ...
                "retrospective_non_repurchase": int(row.non_repurchase_28d),
                "events": len(events),
            }
        )
    con.close()
...
def reports(mode="dev"):
    ...
    snapshots = read(p / "snapshots.json")
    audiences = read(p / "audience_summary.json")
    hill = read(p / "hillstrom/experiment.json")
    uplift = read(p / "hillstrom/uplift.json")
    directory = p / "reports"
    directory.mkdir(exist_ok=True)
    metrics = []
        ...
        metrics.append(
            {
                ...
                "run_id": p.name,
                "run_mode": mode
                if dataset == "rees46_cosmetics"
                else "full_independent",
                "as_of_timestamp": t,
                "window_start": t,
                "window_end_exclusive": str(
                    (pd.Timestamp(t) + pd.Timedelta(days=28)).date()
                )
                if t and split != "latest_unlabeled"
                else None,
                "split": split,
                "numerator": numerator,
    ...
    for row in snapshots:
        add(
            "eligible_buyers_" + row["split"],
            row["n"],
            "observed_customers",
            split=row["split"],
            method="At least one purchase in prior 60 days",
            source="snapshots.json",
        )
        add(
            "non_repurchase_prevalence_" + row["split"],
            row["non_repurchase_prevalence"],
            "fraction",
            split=row["split"],
            denominator=row["n"] if row["label_mature"] else None,
            method="Mean mature 28-day non-repurchase",
            source="snapshots.json",
            evidence="OBSERVED" if row["label_mature"] else "NOT_AVAILABLE",
        )
    for model, result in tests.items():
        for key in [
            ...
            add(
                f"{model}_{key}",
                result[key],
                method=f"Frozen {model}; sklearn {key}",
                source="test_metrics.json",
                denominator=result["n"],
                uncertainty=result["uncertainty"]["intervals"].get(key),
            )
        for row in result["top_k"]:
            add(
                f"{model}_lift_top_{int(row['fraction'] * 100)}pct",
                row["risk_ranking_lift"],
                "ratio",
                method="Top-K precision / same-cohort prevalence; deterministic ID tie break",
                source="test_metrics.json",
                denominator=row["k"],
                numerator=row["positive_count"],
            )
    for row in audiences:
        add(
            "candidate_count_" + row["split"],
            row["selected"],
            "observed_customers",
            split=row["split"],
            method="Frozen capacity, exclusions and risk ranking; consent unknown",
            source="audience_summary.json",
            denominator=row["eligible"],
        )
    for arm in hill["arm_summary"]:
        for key in ["visit_rate", "conversion_rate", "mean_spend_per_assigned"]:
            add(
                "hillstrom_" + arm["arm"] + "_" + key,
                arm[key],
                unit="dollars_per_assigned" if key.startswith("mean") else "fraction",
                dataset="hillstrom_email",
                split="randomized_all",
                denominator=arm["n"],
                method="Assigned-customer mean, 14-day outcome",
                source="hillstrom/experiment.json",
            )
    for row in hill["effects"]:
        add(
            "hillstrom_" + row["arm"] + "_" + row["outcome"] + "_difference",
            row["difference"],
            unit="absolute_difference",
            dataset="hillstrom_email",
            split="randomized_all",
            method=hill["interval_method"],
            source="hillstrom/experiment.json",
            uncertainty=row["ci95_unadjusted"],
        )
    add(
        "hillstrom_visit_qini",
        uplift["qini_auc"],
        dataset="hillstrom_email",
        split="test",
        method="sklift.metrics.qini_auc_score, fixed T-learner visit rankings",
        source="hillstrom/uplift.json",
        uncertainty=uplift["qini_ci95"],
    )
    add(
        "source_rows",
        sum(x["n"] for x in quality["monthly_full_source"]),
        "source_events",
        split="source_coverage",
        method="Count every row in original five archives, before dev sampling",
        source="quality.json",
    )
    manifest = {
        "generated_at": now(),
        "dataset_separation": "never joined",
    ...
    table = pd.DataFrame(
        [
            {k: r[k] for k in ["metric_id", "value", "dataset_id", "split", "unit"]}
            for r in metrics
        ]
    )
    ...
    (directory / "executive_memo.md").write_text(
    ...
    (directory / "hillstrom_readout.md").write_text(
        "# Hillstrom randomized email experiment — independent dataset\n\n"
    ...
    scores = pd.read_parquet(p / "scores_test.parquet")
    outcomes = pd.read_parquet(p / "outcomes_test.parquet")
    policies = []
    for name in ["recency", "risk", "value", "risk_value"]:
        selected = audience(scores, int(np.ceil(len(scores) * 0.1)), name).merge(
            outcomes[["user_id", "non_repurchase_28d"]],
            on="user_id",
            validate="one_to_one",
        )
        policies.append(
            {
                "policy": name,
                "k": len(selected),
                "non_repurchase_rate": float(selected.non_repurchase_28d.mean()),
                "mean_historical_value_proxy": float(
                    selected.historical_value_proxy.mean()
                ),
            }
        )
    ...
    for split in CUTS:
        pd.read_parquet(p / f"scores_{split}.parquet").to_parquet(
            bi / f"customer_scores_{split}.parquet", index=False
        )
...
def presentation(mode="dev"):
    ...
    node = (
        os.environ.get("RETENTION_NODE")
        or shutil.which("node")
        or "/Users/nik/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node"
    )
    subprocess.run([node, str(ROOT / "scripts/build_slides.mjs"), mode], check=True)
    subprocess.run(
        [sys.executable, str(ROOT / "scripts/convert_slides.py"), mode], check=True
    )
