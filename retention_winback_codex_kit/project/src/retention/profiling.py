...
def profile(mode="dev"):
    ...
    if source["status"] != "acquired":
        raise RuntimeError(source.get("resume", "Cosmetics unavailable"))
    con = connect(mode)
    reports = []
    ...
    for i, file in enumerate(source["files"]):
        ...
        con.execute(
            f"CREATE OR REPLACE TEMP TABLE raw_month AS SELECT row_number() OVER () AS source_row, * FROM read_csv('{path}',all_varchar=true,parallel=false)"
        )
        ...
        if not expected.issubset(names):
            raise ValueError(f"Schema mismatch: {names}")
        q = (
            con.execute("""SELECT count(*) n, count(*) FILTER(WHERE user_id IS NULL) missing_user,
        ...
        q["event_counts"] = dict(
            con.execute(
                "SELECT event_type,count(*) FROM raw_month GROUP BY 1"
            ).fetchall()
        )
        reports.append(q)
        sample = (
            "AND ('0x'||substr(md5(user_id),1,8))::UBIGINT % 10=0"
            if mode == "dev"
            else ""
        )
        ...
        con.execute(
            (
                ...
                if i == 0
                else "INSERT INTO source_events "
            )
            + select
        )
    ...
    FROM source_events""")
    con.execute(
        "CREATE OR REPLACE TABLE int_behavior_events AS SELECT * FROM stg_cosmetics_events WHERE valid_behavior"
    )
    counts = (
        con.execute(
            "SELECT count(*) source_rows,count(*) FILTER(WHERE valid_behavior) included,count(*) FILTER(WHERE NOT valid_behavior) excluded,count(*)-count(DISTINCT content_hash) identical_excess FROM stg_cosmetics_events"
        )
        .df()
        .iloc[0]
        .to_dict()
    ...
    sensitivity = (
        con.execute("""SELECT sum(price) FILTER(WHERE valid_price) preserved_value,
    ...
    count(DISTINCT(user_id,user_session)) provided_session_proxy FROM int_behavior_events WHERE event_type='purchase'""")
        .df()
        .iloc[0]
        .to_dict()
    )
    sensitivity["deduplicated_value"] = con.execute(
        "SELECT sum(price) FILTER(WHERE valid_price) FROM (SELECT DISTINCT ON(content_hash) * FROM int_behavior_events WHERE event_type='purchase')"
    ).fetchone()[0]
    daily = con.execute(
        "SELECT cast(event_time AS DATE) AS day,count(*) n FROM int_behavior_events GROUP BY 1 ORDER BY 1"
    ).df()
    daily.to_csv(run_dir(mode) / "coverage_daily.csv", index=False)
    expected_days = 152
    coverage_ok = (
        len(daily) == expected_days
        and str(daily.day.min().date()) == "2019-10-01"
        and str(daily.day.max().date()) == "2020-02-29"
    )
    report = {
        "dataset_id": "rees46_cosmetics",
        "run_mode": mode,
        "monthly_full_source": reports,
        "analytical_waterfall": counts,
        "duplicate_sensitivity": sensitivity,
        "coverage_usable": coverage_ok,
        "coverage_start": "2019-10-01",
        "coverage_end_exclusive": "2020-03-01",
        "coverage_basis": "All five monthly archives, publisher monthly coverage, daily event presence. Does not establish absence of intra-day tracking outages.",
        "duplicate_policy": "Preserve multiplicity; binary targets and purchasing-day frequency robust to exact duplicates.",
    ...
    save(run_dir(mode) / "quality.json", report)
    con.close()
    return report
